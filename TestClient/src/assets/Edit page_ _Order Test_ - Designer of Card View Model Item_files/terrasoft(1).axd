...
(function() {
	...
	var isStrict = document.compatMode == "CSS1Compat", isOpera = ua
			.indexOf("opera") > -1, isSafari = (/webkit|khtml/).test(ua), isChrome9 = isSafari && (/chrome\/9/).test(ua),
			isSafari3 = isSafari, isAppleSafari = ((/applewebkit|khtml/).test(ua) && ua.indexOf("chrome") == -1)
			&& ua.indexOf("webkit/5") != -1, isIE = !isOpera
			&& ua.indexOf("msie") > -1, isIE7 = !isOpera && (document.documentMode == 7 ||
				(/msie 7/).test(ua) && !document.documentMode)
	...
	Ext.isAppleSafari = isAppleSafari;
	Ext.isChrome9 = isChrome9;
	ieVer = navigator.userAgent.match(/msie (\d+)/i);
	ieVer = ieVer ? parseInt(ieVer[1], 10) : 0;
	Ext.isWebKit = ua.indexOf(' applewebkit/') > -1;
	isIE6 = isIE && ieVer <= 6;
	isIE7 = isIE && ieVer == 7;
	isIE8 = isIE && ieVer == 8;
	isIE9 = isIE && ieVer >= 9;
...
Ext.ns("Ext", "Ext.util", "Ext.treegrid", "Ext.dd", "Ext.tree", "Ext.data",
	...
	Ext.lib.Event = function() {
		...
		return {
			...
			removeListener : function(S, O, R) {
				var Q, N;
				// TODO: хак для устранения ошибки в IE при попытке отписаться от события unload для закрытого окна
				if (Ext.isIE) {
					try {
						S = Ext.getDom(S);
					} catch(X) {
			...
			doAdd : function() {
				if (window.addEventListener && !Ext.isIE9) {
			...
			doRemove : function() {
				if (window.removeEventListener && !Ext.isIE9) {
					return function(O, M, N, L) {
